...
export default {
  ...
  methods:{
    ...
    query(row){
      ...
      const {id,name,age,phone} = this.productCarList[this.productCarList.findIndex(item => item.id === row.id)]
      ...
      this.$confirm('确认删除？','提示',{
        ...
        type: 'warning'
         ...
         http({
          ...
          if(res.code==200){
            _this.productCarList.splice(_this.productCarList.findIndex(item => item.id == row.id), 1)
            this.$message({
              type: 'success',
              message: '删除成功!'
            });
...
function getAllUser(_this){
  http({
    //这里是你自己的请求方式、url和data参数
    method: 'get',
    url: 'http://localhost:3000/shopcar/queryAll',
    data: {}
  }).then(function (res) {
    console.log(res);
    _this.productCarList = res.data;
  }).catch(function (err) {
    console.log(err);
  });
}
